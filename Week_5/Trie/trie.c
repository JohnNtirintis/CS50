...
bool check(char* word)
{
    ...
    for(int i = 0; i < strlen(word); i++){
        ...
        // By converting all letters to lowercase and,
        // subtracting by 'a' or 97 in ASCII
        // we can tell by the remainder of the value
        // which letter it is
        int index = tolower(word[i]) - 'a';

        // Checks to see if the index is a char within the enlgish alphabet
        // Using ASCII
        // In essence its checking user input.
        if(index < 0 || index >= SIZE_OF_ALPHABET){
            return false;
        }
        // Checks to see if there are any children for the current letter
        // If there arent any children then, that word isnt part of the .txt file
        // and it returns false
        if(cursor->children[index] == NULL){
            return false;
        }

        // Continues searching the children (next letters)
        cursor = cursor->children[index];
    }
